...
Original: [[raw/unix.foo/the-insecurity-of-debian]]

In June of 2023 [[wiki/entities/red-hat|Red Hat]] made a controversial decision to change how they distribute the source code behind [[wiki/entities/red-hat-enterprise-linux|Red Hat Enterprise Linux]] (RHEL). There have been a lot of keyboards tapped angrily across social media that left many uncertain about the ramifications of the decision. There were many questions about the future viability of downstream rebuilds of RHEL affecting distributions like Rocky Linux, AlmaLinux, Oracle Linux, and others. Each have since made announcements to try and calm their communities.

Still. Many in the open source community have interpreted Red Hat’s decision for what it really was: A dick move.

There has been a steady uptick of people stating that they will migrate (or already have) to [[wiki/entities/debian|Debian]] – seeking refuge from what they see as greedy corporate influence. I understand the sentiment fully. However, there’s a problem here that I want to talk about: security.
...
Long ago, Red Hat embraced the usage of [[wiki/concepts/selinux|SELinux]] . And they took it beyond just enabling the feature in their kernel. They put in the arduous work of crafting default SELinux policies for their distribution.
...
On Red Hat based distributions, you can use a drop-in [[wiki/entities/docker|Docker]] alternative named [[wiki/entities/podman|podman]] which allows you to run containers without needing a daemon (unlike Docker) and it provides other benefits like being able to run fully root-less. But Red Hat takes it a step further here and applies strong default SELinux policies that separate the container from the host OS and even from other containers!
...
However, its default security framework leaves much to be desired. Debian’s decision to enable [[wiki/concepts/apparmor|AppArmor]] by default starting with version 10 signifies a positive step towards improved security, yet it falls short due to the half-baked implementation across the system.
...
The fundamental difference between AppArmor and SELinux lies in their approach to [[wiki/concepts/mandatory-access-control|Mandatory Access Control (MAC)]]. AppArmor operates on a path-based model, while SELinux employs a significantly more complex type enforcement system. This distinction becomes particularly evident in container environments.
...
But SELinux doesn’t stop at type enforcement. It takes container isolation a step further with [[wiki/concepts/multi-category-security|Multi-Category Security (MCS)]] labels. These labels function as an additional layer of segregation, ensuring that even containers of the same type (**container_t**) remain isolated from each other. Each container gets its own unique MCS label, creating what amounts to a private sandbox within the broader **container_t** environment.
...
- Red Hat distributions ship with default SELinux policies that actively protect critical daemons. ([[raw/unix.foo/the-insecurity-of-debian#Introduction]]) — "These policies ship enabled by default in their distribution. The policies help protect a variety of daemons that run by default on RHEL"
- Debian leaves many services unprotected because it ships with minimal AppArmor profiles. ([[raw/unix.foo/the-insecurity-of-debian#Debian’s Approach]]) — "Debian ships with a minimal set of AppArmor profiles, leaving many critical services unprotected."
- Docker's default AppArmor profile leaves significant attack vectors available. ([[raw/unix.foo/the-insecurity-of-debian#Debian’s Approach]]) — "This profile, while providing some protection, leaves significant attack surfaces exposed."
- SELinux improves container isolation by combining type enforcement with Multi-Category Security (MCS) labels. ([[raw/unix.foo/the-insecurity-of-debian#AppArmor vs. SELinux]]) — "SELinux doesn’t stop at type enforcement. It takes container isolation a step further with Multi-Category Security (MCS) labels."
