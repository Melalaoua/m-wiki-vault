...
- Some small requests on quality of life using the app.
...
### May, the 10th
I used claude Design to redefine the UI of the OAD app. I'm satisfied with the proposed content. Much better than what I did. I'll try implementation with claude Code now.
...
- Polish UI, clarify diagnostic flow.
- Polish docs (pathologies, labtests).
- Add missing pieces on various pages (side panel in patient Step)
- Add requested features from Mel
- Add requested features from superiors : labtests numerical values, auto-toggle symptoms based on entry.
- Symptom grouping is bugged
...
TODO : Add notes to alembic and sqlAlchemy tables in OAd Server
...
Swap landscape/portrait = vide le formulaire 
Poids : mettre 0 
enlever urgence fin
envoyer mail pour publication ce week end
contenu mail : google docs, patients tests, limitation base de données, tjr pas acces au whats app + réunion salim + convenir une date pour retour + je vais gérer la partie tests + traduction viendront à la fin + license RxDb + serveur allumé + quand un patient/consultation est delete + mode portrait pour consultation


adb logcat -c
adb logcat > crash.log
close/ reopen the app
grep -iE "FATAL|AndroidRuntime|ReactNative|Exception" crash.log
...
2. **From your machine** (what I just did — fastest, scriptable):
   ```bash
...
A few notes so this doesn't bite you again:
- `adb reverse` is **cleared whenever you unplug the phone or restart adb**, so re-run it after reconnecting. `npx expo run:android` sets it automatically at the end of a build — that's why builds "just work" but a manual relaunch doesn't.
- Check the phone screen now — it should show your app loading. If you instead get a red error screen, tell me what it says and I'll dig in.

Want me to add a small npm script (e.g. `pnpm dev:usb`) that runs the reverse + deep-link so you can reconnect with one command?
